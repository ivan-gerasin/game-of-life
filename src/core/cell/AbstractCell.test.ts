...
describe('Cell', () => {
	let worldMock: IWorld<IBaseCellFactory, IBaseCell>,
		cell: ICell<IBaseCellFactory, IBaseCell>
	...
	describe('positions of neighbours', () => {
		test('atTop', () => {
			cell.atTop()
			expect(worldMock.at).toBeCalledWith(
		...
		test('atTopRight', () => {
			cell.atTopRight()
			expect(worldMock.at).toBeCalledWith(
				new Point(cellPosition.x + 1, cellPosition.y - 1)
			)
		})
		test('atTopLeft', () => {
			cell.atTopLeft()
