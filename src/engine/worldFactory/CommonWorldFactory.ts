...
export default class CommonWorldFactory implements ICommonWorldFactory {
	...
	build<F extends ICellFactory<F, C>, C extends ICell<F, C>>(
		cellFactory: F,
	...
	buildWithPreset<F extends ICellFactory<F, C>, C extends ICell<F, C>>(
		cellFactory: F,
		preset: Preset,
		symbolMapper: ISymbolToCellMapper<F, C>,
		size = this.DEFAULT_SIZE
	): IWorld<F, C> {
