...
export default class Herbivore extends AbstractCell<ILivePlanetCellFactory, ILivePlanetCell> implements ILivePlanetCell {
  ...
  nextGeneration(): ILivePlanetCell {
    ...
    const amountOfFood = neighbours.filter(type => type === LiveCellType.Plant).length
    const rivals = neighbours.filter(type => type === LiveCellType.Herbivore).length
    ...
    const predatorsAround = neighbours.filter(type => type === LiveCellType.Predator).length
